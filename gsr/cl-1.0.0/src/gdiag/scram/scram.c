...
struct SCRAMNET *init_scramread(init)
struct SCRAMNET *init;
{
  const int ttl=64;
  int itmp;
  ...
  itmp = TRUE;
  if( setsockopt(scram->sock, SOL_SOCKET, SO_REUSEADDR, &itmp, sizeof(int)) <0)
