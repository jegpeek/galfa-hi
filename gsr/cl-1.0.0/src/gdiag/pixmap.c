...
static int key_list[KEYBUFSIZ];
static int key_head=0;
static int key_tail=0;

static pthread_mutex_t pixmap_mutex = PTHREAD_MUTEX_INITIALIZER;
...
readch(void)
{
    ...
    if (fb->vnc) {
        if (key_head == key_tail)
            return 0;
        ch = key_list[key_tail++];
        if (key_tail == KEYBUFSIZ)
            key_tail = 0;
        return ch;
    }
#endif
...
console_close(void)
{
    if (con_fd != -1) {
        ...
        if (!is_console(tty_fd))
	    if (ioctl(con_fd, VT_ACTIVATE, 1))
	        perror("VT_ACTIVATE");
...
console_open(void)
{
    ...
    tty_fd = open("/dev/tty", O_RDWR | O_NDELAY);
    if (tty_fd < 0) {
...
{
    if (down) {
        if (((key_head+1) & (KEYBUFSIZ-1)) == key_tail)
            return;
        key_list[key_head++] = k;
        if (key_head == KEYBUFSIZ)
            key_head = 0;
    }
}
...
myRun(void *data)
{
    ...
    while (1) {
        pthread_mutex_lock(&pixmap_mutex);
        rfbProcessEvents(rfbScreen, 0);
        pthread_mutex_unlock(&pixmap_mutex);
    }
}

void
pixmap_init_vnc(int *ac, char **av, int width, int height)
{
    pthread_t mythread;

    fb = pixmap_new(width,height);
    pixmap_setcolor(fb, C_BLACK);
    fb->vnc = rfbGetScreen(ac, av, width, height, 5, 3, 2);
    fb->vnc->frameBuffer = (char *) fb->buf;
    fb->vnc->rfbAlwaysShared = TRUE;
    fb->vnc->kbdAddEvent = dokey;

    // Setup for 565 RGB
    //
    fb->vnc->rfbServerFormat.redMax = 31;
    fb->vnc->rfbServerFormat.greenMax = 63;
    fb->vnc->rfbServerFormat.blueMax = 31;
    fb->vnc->rfbServerFormat.redShift = 11;
    fb->vnc->rfbServerFormat.greenShift = 5;
    fb->vnc->rfbServerFormat.blueShift = 0;
    fb->vnc->desktopName = "Pixmap framebuffer";
    fb->vnc->httpDir = "/etc/classes";
    fb->vnc->httpEnableProxyConnect = TRUE;

    rfbInitServer(fb->vnc);
    rfbUndrawCursor(fb->vnc);

    // rfbRunEventLoop doesn't seem to work very well, it
    // doesn't listen to http and seems to crash a lot
    // rfbRunEventLoop(fb->vnc, -1, TRUE);  // background loop
    
    pthread_create(&mythread, NULL, myRun, fb->vnc);
}
#endif
...
pixmap_init(void)
{
    ...
    // msg("yres:   %d\n", fb->ys);
    // msg("stride: %d\n", fb->stride);
    // Wrong, but lets me see offscreen allocations for 640x480 system
...
#ifdef PM_VNC
    pixmap_vnc_mark_rect(pmd, dxo, dyo, xs, ys);
#endif
}
...
#ifdef PM_VNC
    pixmap_vnc_mark_rect(pm, xo, yo, xs, ys);
#endif
}

#ifdef PM_VNC
void
pixmap_vnc_mark_rect(pixmap *pm, int xo, int yo, int xs, int ys)
{
    if (pm->vnc) { 
        while(pm->parent) {
            xo += pm->parent_xo;
        ...
        pthread_mutex_lock(&pixmap_mutex);
        rfbMarkRectAsModified(pm->vnc, xo, yo, xo+xs, yo+ys);
        pthread_mutex_unlock(&pixmap_mutex);
    }
}
#endif
