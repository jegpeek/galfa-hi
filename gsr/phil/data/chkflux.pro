...
for i=0,j-1 do begin &$
	...
	if i eq 0 then begin &$
		...
		color=col,xtitle='log10 (freq [Mhz])',$
		ytitle='log10(flux [Jy])',$
title='source flux vs frequency for Csalters and idl flux files' &$
	endif else begin &$
		oplot,alog10(freq),alog10(fluxsrc(nm,freq)),color=col &$
	endelse &$
	note,ln+i,nm,xp=xp,color=col &$
endfor
...
for i=0,j-1 do begin &$
	col=i+1 &$
	nm=sflux[inddif[0,i]].name &$
	oplot,alog10(freq),alog10(fluxsrc(nm,freq)),color=col,linestyle=2 &$
endfor
flag,alog10(1420.)
note,3,'<-- 1420Mhz',xp=.6
end
