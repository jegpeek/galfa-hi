...
pro cormonall,lun,sl=sl,norms=norms,nospectra=nospectra,noimg=noimg,step=step,$
               col=col,pltmsk=pltmsk,vel=vel,pol=pol,imgpol=imgpol,ravg=ravg,$
...
;-----------------------
    ...
    sluse=0
    if n_elements(sl) gt 1 then begin
        sluse=1
        slnumscans=n_elements(sl)
        slscan=0
        slrec =0
    endif
...
;
        ...
        if dorms and (nrecsB[indB] ge minRecsRms) then begin
            ...
            corplot,brms,m=pltmsk,vel=vel,pol=lpol,extitle=' SCAN RMS'
            ver
        endif
;
;       do img 
            ...
            if (curScanType eq posOffScan) and $
               (lastScanType eq posOnScan) then begin
                ...
                img=corimgdisp(bb[0:nrecs-1,0],wxlen=imgxlen,wylen=imgylen,$
                           ...
                           col=col,ravg=ravg,median=imgmedian)
            endelse
        endif
;
...
;
        if doavgspc and (recsavged gt 1) then begin
            ...
            if base ne 0 then begin
              ...
              corplot,bl,m=pltmsk,vel=vel,pol=lpol,$
                        extitle=' Scan Avg'
            endif else begin
                corplot,bavg,m=pltmsk,vel=vel,pol=lpol,$
                        extitle=' Scan Avg'
            endelse
        endif
...
;
     ...
     if dospectra then begin
		...
		endelse
        wset,winspectra
        corplot,b,m=pltmsk,vel=vel,pol=lpol,extitle=' Single Rec'
     endif
;
