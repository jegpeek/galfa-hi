...
;   create the cal structure to return
    ...
    if nrecs gt 1 then begin
        ...
        if keyword_set(han) then begin
            ...
            if keyword_set(blda) then coff=caloff
        endelse
    endelse
;
...
;
    for i=0,numbrds-1 do begin
        npol=(calon[0].(i).p[1] eq 0) ? 1:2
        polB1st=calon[0].(i).p[0] eq 2
        istat=corhcalval(calon[0].(i).h,calvalLoc)
...
;
		...
		calValAr[*,i]=calvalloc
;
;   compute mean value ,scale
;
        ind=where(mask.(i)[*,0] gt .1,count)
        caldifTp[0,i]=total(caldif.(i).d[ind,0])/count
        calscl[0,i]=calvalLoc[0]/caldifTp[0,i]
        if npol gt 1 then begin
            ind=where(mask.(i)[*,1] eq 1,count)
            caldifTp[1,i]=total(caldif.(i).d[ind,1])/count
            calscl[1,i]=calvalLoc[1]/caldifTp[1,i]
        endif
    endfor
