...
function corimgonoff,lun,scan,sl=sl,ver=ver,col=col,han=han,red=red,clip=clip,$
                     bon=bon,boff=boff,cont=cont,_extra=e
;
    ...
    img=corimgdisp(ball,bpc=bpc,yrange=[ymin,ymax],col=col,clip=clip,_extra=e)
    ver,ymin,ymax
    hor
    ...
    for i=0,nbrds-1 do begin
        !p.multi=[nbrds-i,1,nbrds]
        frq=corfrq(ball[0].(i).h)
        if (ball[0].(i).h.cor.numsbcout eq 2) then begin
            y=(total(ball[0:nrecson-1].(i).d[*,0],2) + $
               total(ball[0:nrecson-1].(i).d[*,1],2))/ $
              (total(ball[nrecson:nrecson+nrecsoff-1].(i).d[*,0],2) + $
               total(ball[nrecson:nrecson+nrecsoff-1].(i).d[*,1],2))
        endif else begin
            y=total(ball[0:nrecson-1].(i).d[*,0],2)/ $
              total(ball[nrecson:nrecson+nrecsoff-1].(i).d[*,0],2)
        endelse
