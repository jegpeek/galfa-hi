...
;
    ...
    useinpmask= n_elements(masktouse) gt 0 ; instead of edgefraction
...
;
    coefI={     pol   :intarr(2,brddim)    ,$;1 if this sbc,pol in use   
                deg   : 0             ,$;polynomial fit order
                fsin  : 0             ,$;harmonic fit order
                nlags :intarr(brddim)      ,$;chns each sbc
                coefAr:fltarr(sinOrderTot*2+polyOrderTot+1,2,brddim),$;coef from fit 
                rms   : fltarr(2,brddim)   ,$; of fit inside the mask
                maskFract:fltarr(2,brddim)}  ;fraction of band used for fit
    coefI.deg=deg
    coefI.fsin=fsin
;
        ...
        for ipol=0,npol-1 do begin
            if useinpmask then begin
				if useRaw then begin
					...
					ind0=where(masktouse.(ibrd)[*,ipol] ne 0.)
				endelse
			endif
