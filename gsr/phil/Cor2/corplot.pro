...
;04jul04 - use xyouts to position title rather than note.
...
;;           for ii=0,(size(b))[1]-1 do begin ; if array loop over whole array
             for ii=0,nrecs-1 do begin ; if array loop over whole array
...
;
                        if bychn then begin
                           ...
                           x=corfrq(b[ii].(i).h,retvel=vel,retrest=rest)
                        endelse
                        if  over then begin
                            !p.multi=[plotsleft,across,down]
                        endif 
...
'("pwrRatio:",f4.1," ",f4.1)',b[ii].(i).h.cor.lag0pwrratio[0]*accumscl[i],$
                            ...
                            if (labcnt lt across) then begin
                                if not nolab then $
                                note,ln,lab,charsize=cs,color=colphLoc[1]
                            endif else begin
                                if not nolab then $
                                note,ln+15,lab,charsize=cs,color=colphLoc[1]
                            endelse
                            labcnt=labcnt+1
                        endif
                        title=""
