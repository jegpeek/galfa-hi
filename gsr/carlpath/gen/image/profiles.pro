pro profiles, image, ORDER=order, AXIS=axis, $
              SX=sx, SY=sy, $
...
if not keyword_set(AXIS) then begin
    x0 = (N_elements(SX) eq 0) ? 0L : long(sx)
    y0 = (N_elements(SY) eq 0) ? 0L : long(sy)
endif else begin
...
xpix = (xcrange[1]-xcrange[0])*findgen(nx)/(nx-1) + xcrange[0]
ypix = (ycrange[1]-ycrange[0])*findgen(ny)/(ny-1) + ycrange[0]
...
; MAKE NEW WINDOW...
window, /FREE, XSIZE=xsize, YSIZE=ysize, TITLE='Profiles' 
new_w = !d.window
