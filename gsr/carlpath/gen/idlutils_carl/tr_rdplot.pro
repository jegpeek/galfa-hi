...
pro tr_RDPLOT, x, y, WaitFlag, DATA=Data, DEVICE=Device, NORMAL=Normal, $
   ...
   COLOR=Color, BACKGROUND=BackGround, CROSS=Cross
   
;*******************************************************************************
;+
;NAME:
;RDPLOT (tr_rdplot) -- like CURSOR but with full-screen cursor and 
;	continuous readout.
;
; PURPOSE:
...
; license is renewed!}
; NOTE: Robishaw fixed this bug 1/27/05.
;
   ...
   if not(Decomposed) then begin
       ...
       if (Color ge !D.Table_size) then $
         message, 'Trying to draw cursor with color table index GT Table Size', /INFO
       DevColor = TTab[Color < (!D.Table_size - 1)]
       if (BackGround ge !D.Table_size) then $
         message, 'Specified background has color table index GT Table Size', /INFO
...
;
   ...
   if Down then ExitFlag = (!mouse.button gt 0) and (Err0 eq 0) $
           else ExitFlag = (!mouse.button gt 1) or Change
