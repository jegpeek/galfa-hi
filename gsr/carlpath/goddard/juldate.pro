...
;-
 ...
 if ( N_params() EQ 0 ) and (not keyword_set( PROMPT ) ) then begin
     ...
     print,'  date - 3-5 element vector containing [year,month,day,hour,minute]'
     print,'  jd - reduced julian date (double precision)'
     return
 endif
 ...
 if ( N_elements(date) EQ 0 ) then begin   
    ...
    opt = ''                                                          
    rd: read,' Enter Year,Month,Day,Hour & Minute (All Numeric): ',opt
    date = getopt( opt, 'F' )
 ...
 iy = fix( date[0] )                     
 im = fix( date[1] )
 day = double(date[2]) + ( date[3] + date[4]/60.0) / 24.0
