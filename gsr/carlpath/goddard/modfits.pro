pro MODFITS, filename, data, header,SILENT = silent, EXTEN_NO = exten_no
;+
; NAME:
;       MODFITS
; PURPOSE:
...
; EXPLANATION:
;       The updated header or array cannot change the size of the 
;       FITS file.
;
; CALLING SEQUENCE:
;       MODFITS, Filename, Data, [ Header, /SILENT, EXTEN_NO = ]
;
; INPUTS:
...
;
;       HEADER - FITS header (string array) to be inserted into the FITS file.
;
; OPTIONAL INPUT KEYWORDS:
;       EXTEN_NO - scalar integer specify the FITS extension to modified.  For
...
;               IDL> sxaddpar,h,'DATE','22/03/94'   ;Modify value of DATE 
...
;       would not be altered by a multiple of 2880 bytes.)    MODFITS is best
...
; RESTRICTIONS:
;       (1) Cannot be used with random groups or variable length binary tables
;
; PROCEDURES USED:
;       Functions:   SXPAR, FXPOSIT
...
; Open file and read header information
   ...
   if nheader GT 1 then begin
      ...
      if (nheader mod 36) NE (Noldheader mod 36) then begin
   ...
   if ndata GT 1 then begin
        ...
        if (newbytes mod 2880) NE (nbytes mod 2880) then begin
        ...
        Little_endian = ( !VERSION.ARCH EQ "mipsel" ) or $ 
                        ( !VERSION.OS EQ 'windows') or (!VERSION.OS EQ 'Win32')
