...
;
; NOTES:
;	If a list vector is not supplied, then the found objects are displayed
;	at the terminal.
;
; EXAMPLE:
...
;	GET_EQUINOX(), GSSSADXY, GSSSXYAD, SXPAR(), XY2AD 
; REVISION HISTORY:
...
; Check if catalogue has preselected output fields
 ...
 dbopen,catalogue
 if !ERR lt 0 then return               ;Was database found?
...
; Compare equinox of image with that of database and precess if necessary
 ...
 if ( code EQ -1 ) then im_year = 1950.    ;Assume image in 1950 EQ as default
 if ( im_year NE cat_year ) then begin 
      crval_ra = astr.crval[0] & crval_dec = astr.crval[1]
      precess,crval_ra,crval_dec,im_year,cat_year
      astr.crval = [crval_ra,crval_dec]
 endif
