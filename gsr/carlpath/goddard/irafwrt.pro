...
;	(3)  IRAFWRT has only been tested on SunOS, Solaris and VMS systems.
;
; PROCEDURES CALLED:
;	FDECOMP, IS_IEEE_BIG(), MINMAX(), REPCHR(), STRN(), SXDELPAR, SXPAR()
...
; Get the dimensions, vector of dimensions and the data type
  ...
  type = imsize[naxis+1]
  min_max = float(minmax(image))  ; find the minimum and maximum pixel values
...
;
 ...
 min = strn(min_max[0],format = '(E13.6)')
 max = strn(min_max[1],format = '(E13.6)') 
...
;
  ...
  hdr[120] = byte(min_max[1],0,4)
  hdr[124] = byte(min_max[0],0,4)
  cd,current = dir
  ...
  if keyword_set(pixdir) then dir = strlowcase(pixdir)
  pixname = strlowcase( host+'!' + dir + fname + '.pix')
  len1 = strlen(pixname)
  len2 = strlen(header)
