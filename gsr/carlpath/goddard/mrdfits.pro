...
if fxpar(header, 'XTENSION') ne 'TABLE   ' then begin
...
if not keyword_set(no_fix_varcols) then begin
    ...
    corres = corres[where(w gt 0)]
    fnames = fnames[where(w gt 0)] 
    fvalues = fvalues[where(w gt 0)]
    scales = scales[where(w gt 0)]
    offsets = offsets[where(w gt 0)]

    vcols = vcols[where(vdims gt 0)]
    vtypes = vtypes[where(vdims gt 0)]
    vdims = vdims[where(vdims gt 0)]
...
if xten ne 'BINTABLE' and xten ne 'A3DTABLE' then begin 
        print, 'MRDFITS: Header is not from binary table.' 
        status = -1; 
        return 
endif 
...
if  status ge 0  and  scaling  and  size gt 0  then begin
	...
	if w[0] ne -1 then mrd_scale, type, scales, offsets, table, header,$
	    ...
	    dscale=dscale, tempdir=tempdir, silent=silent
endif
