PRO TVLASER, hdr, Image, BARPOS=BarPos, CARROWS=CArrows, CLABELS=CLabels, $
	...
	IMAGEOut=ImageOut, INTERP=Interp, MAGNIFY=Magnify, NoClose=noclose, $
        ...
        XSTART=XStart, YSTART=YStart, XDIM=XDim, YDIM=YDim, TrueColor=TrueColor 
;+
; NAME:
;      TVLASER
; PURPOSE:
...
;
; CALLING SEQUENCE:     
;      TVLASER, [header, Image, BARPOS = ,CARROWS =, CLABELS = ,/COLOPS, 
...
;       Added /NoClose keyword, trim Equinox format  W. Landsman 9-Jul-1997
;-
...
;   If the BarPos keyword has four or five elements, then show the color bar.
 ...
 if (not(TrueValue)) then begin 
   ...
   if ( (NumEls eq 4) or (NumEls eq 5) ) then begin
    ColorBar = rebin(bindgen(256), 256, 20)
    ColorBar[0:*,[0,19]]  = 0
    ColorBar[[0,255],0:*] = 0
...
;SCALE
    XYOUTS,LabX2s,LabYs,['SCALE:',CDELTAS],/NORMAL
    LabYs = LabYs + dY
...
;EXPOSURE TIME   First try 'EXPTIME' then 'INTEG'
	...
	exptime = strtrim(string(exptime,f='(f6.1)'),2) + ' seconds'
