...
;
; RESTRICTIONS:
;	The current algorithm is not particularly efficient. The user must 
;	delete the temporary file under MacOS.
;
...
; Delete the file temp_struct.pro

 case !VERSION.OS_FAMILY of
   'vms' : spawn, 'delete temp_'+fitsname+'.pro;*'
   'MacOS':message,'Please delete file temp_' + fitsname+'.pro ',/INF  
   'Windows': spawn,'del temp_' + fitsname + '.pro'
    else : spawn, '/usr/bin/rm temp_'+fitsname+'.pro'
 endcase
