pro lsfs, path, inputfiles, $
	...
	rffrq_wb, rffrq_nb, caldeflnwb, caldeflnnb, $
	yesnowb=yesnowb, yesnonb=yesnonb, skipwb=skipwb, $
	error=error, quiet=quiet, startndx=startndx
;+
;NAME:
;LSFS -- boss for the lsfs reductions. returns derived parameters given
;the lists of input files thaht contain a group's smartf spectra.
;
;calling sequence:
;lsfs, path, inputfiles, $
;	ggwb, rf4wb, rfwb, fnewwb, multwb, problemwb, $
;	ggnb, rf4nb, rfnb, fnewnb, multnb, problemnb, $
;	bbfrq_nb, bbgain_dft_nb, $
;	rffrq_wb, rffrq_nb, caldeflnwb, caldeflnnb, $
...
;matrices.
;	SKIPWB: skip wb calc for testing nb. don't set
...
;
;OPTIONAL OUTPUTS:
;	ERROR, generated by lsfs1. if nonzero, no solution.
;NOTE ABOUT UNITS:
;
...
m1_to_m1s_s0, path, inputfiles, m1s, startndx=startndx
...
pwroffonwb= total( total( pwrwb_avg,5),3)/(7.*ncycs)
caldeflnwb= pwroffonwb[ *,*,1]- pwroffonwb[ *,*,0]
caldeflnwb= reform( caldeflnwb, 14)
...
pwroffonnb= total( total( pwrnb_avg,5),3)/(7.*ncycs)
caldeflnnb= pwroffonnb[ *,*,1]- pwroffonnb[ *,*,0]
caldeflnnb= reform( caldeflnnb, 14)
