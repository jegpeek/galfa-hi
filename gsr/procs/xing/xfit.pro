pro xfit, root, region, scans, proj, no_auto=no_auto, conrem=conrem, tdf=tdf, keepold=keepold, xingname=xingname
...
;           tdf=tdf, keepold=keepold, xingname=xingname
;
; INPUTS:
...
for k=0l, l-1 do begin
    ...
    fitexy, xaxis[wh1], xarr[k].spect1[wh1], A1, B1, X_SIG=0 , Y_SIG=std1
    fitexy, xaxis[wh2], xarr[k].spect2[wh2], A2, B2, X_SIG=0 , Y_SIG=std2
    endif
   ...
   ;This statement begins the error handler: 
    IF Error_status NE 0 THEN BEGIN 
       ...
       stop     
   ENDIF 
    FITEXY, s2, s1, A, B, X_SIG=std1, Y_SIG=std2, sig_a_b
    outx[k].gainr=B
...
for i=0l, scans-1 do begin
    ...
    for j=i+1, scans-1 do begin   
            ;Check that xday ne 0 to accomodate 2's from links
            if xday[i,j] ne 0 then begin
        loop_bar, tn, total(xday)
        fe = file_exists(root + proj + '/' + region + '/xing/'+ region + string(i, format=scnfmt) + '-'+  string(j, format=scnfmt)  + '_l'+xnus+'.sav')
        if fe eq 0 then print, 'Missing expected file: ' + root + proj + '/' + region + '/xing/'+ region + string(i, format=scnfmt) + '-'+  string(j, format=scnfmt)  + '_l'+xnus+'.sav'
        if fe ne 0 then restore, root + proj + '/' + region + '/xing/'+ region + string(i, format=scnfmt) + '-'+  string(j, format=scnfmt)  + '_l'+xnus+'.sav' else continue
