...
pro scube_diediedie, root, region, proj, cnx, cny, rs=rs, _REF_EXTRA=_extra, tdf=tdf, spname=spname, badrxfile=badrxfile, xingname=xingname, odf=odf, blankfile=blankfile, arcminperpixel=arcminperpixel, pts=pts,  cpp=cpp, madecubes=madecubes
...
sdgw, root, region, proj, 'TOGS-' + string(cnx, f='(I2.2)') + '-' +  string(cny, f='(I2.2)') + '-0-0', scx0, scy0, 0, 8191, 1, [256, 256],rs=rs, projection='CAR', _REF_EXTRA=_extra, tdf=tdf, spname=spname,  badrxfile=badrxfile, xingname=xingname,  savepath=savepath, gridfunc='GAUSS', odf=odf, norm=norm, blankfile=blankfile, arcminperpixel=1., pts=pts, /truecart, cpp=cpp, /nofits, spp=spp
...
sdgw, root, region, proj, 'TOGS-' + string(cnx, f='(I2.2)') + '-' +  string(cny, f='(I2.2)') + '-1-0', scx1, scy0, 0, 8191, 1, [256, 256],rs=rs, projection='CAR', _REF_EXTRA=_extra, tdf=tdf, spname=spname,  badrxfile=badrxfile, xingname=xingname,  savepath=savepath, gridfunc='GAUSS', odf=odf, norm=norm, blankfile=blankfile, arcminperpixel=1., pts=pts, /truecart, cpp=cpp, /nofits, spp=spp
...
sdgw, root, region, proj, 'TOGS-' + string(cnx, f='(I2.2)') + '-' +  string(cny, f='(I2.2)') + '-0-1', scx0, scy1, 0, 8191, 1, [256, 256],rs=rs, projection='CAR', _REF_EXTRA=_extra, tdf=tdf, spname=spname,  badrxfile=badrxfile, xingname=xingname,  savepath=savepath, gridfunc='GAUSS', odf=odf, norm=norm, blankfile=blankfile, arcminperpixel=1., pts=pts, /truecart, cpp=cpp, /nofits, spp=spp
inttime[256:511,256:511] = spp
print, '3/4'

if not keyword_set(norm) then delvar, norm

sdgw, root, region, proj, 'TOGS-' + string(cnx, f='(I2.2)') + '-' +  string(cny, f='(I2.2)') + '-1-1', scx1, scy1, 0, 8191, 1, [256, 256],rs=rs, projection='CAR', _REF_EXTRA=_extra, tdf=tdf, spname=spname,  badrxfile=badrxfile, xingname=xingname,  savepath=savepath, gridfunc='GAUSS', odf=odf, norm=norm, blankfile=blankfile, arcminperpixel=1., pts=pts, /truecart, cpp=cpp, /nofits, spp=spp
...
; the whole shebang
;fullcube = fltarr(512, 512, 8192)
;norms = fltarr(4)
;if not keyword_set(memfact) then begin
;restore, fn00
;norms[0] = norm
;fullcube[256:511,0:255, *] = temporary(cubesav)*norms[0]
;restore, fn10
;norms[1] = norm
;fullcube[0:255,0:255, *] = temporary(cubesav)*norms[1]
;restore, fn01
;norms[2] = norm
;fullcube[256:511,256:511, *] = temporary(cubesav)*norms[2]
;restore, fn11
;norms[3] = norm
;fullcube[0:255,256:511, *] = temporary(cubesav)*norms[3]
;endif

;fns = [fn00, fn10, fn01, fn11]
;norms = fltarr(4)
;if keyword_set(memfact) then begin
;    for i=0, 3 do begin
;        restore, fns[i]
;        norm[i] = norm
;        for j=0, 2^memfact-1 do begin
;            fullcube[256*((i+1) mod 2):256*((i+1) mod 2) + 255, floor(i/2)*256:floor(i/2)*256 + 255, 2^(13.-memfact)*j:2^(13.-memfact)*(j+1)-1] = cubesav[0:255, 0:255, 2^(13.-memfact)*j:2^(13.-memfact)*(j+1)-1]*norm[i]
;        endfor
;        cubesav=0.
;    endfor
;endif
;fullcube = temporary(fullcube)/mean(norms)
...
sxaddpar, hdr, 'OBJECT', 'GALFA-HI RA+DEC Tile ' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)') + 'Integration Time Map'
...
Tname = 'GALFA_HI_RA+DEC_' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)') + '_T.fits'
fits_write, Tname, inttime, hdr

restore,  getenv('GSRPATH') + 'savfiles/hdr.sav'
sxaddpar, hdr, 'NAXIS3', 2048
sxaddpar, hdr, 'OBJECT', 'GALFA-HI RA+DEC Tile ' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)')
sxaddpar, hdr, 'CRPIX1', 256.5 - (180.0 - cx)/0.01666666
sxaddpar, hdr, 'CRPIX2', 256.5 - cy/0.01666666
...
spawn, 'mkdir ' + 'GALFA_HI_RA+DEC_' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)')
cd, 'GALFA_HI_RA+DEC_' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)')
for i=0, 511 do begin
    loop_bar, i, 511
    hdr0 = hdr
    sxaddpar, hdr0, 'OBJECT', 'GALFA-HI RA-VEL Slice ' + string(cx, f='(I3.3)') + '.00+' +  string(cy, f='(I2.2)') + '.' + string(ceil((cy - floor(cy))*100), f='(I2.2)') + ' ' + string(i+1, f='(I3.3)')
...
;read and write Narrow cube
    Narrow = readfits(Nname, hdrN, /silent)
...
;read and write Wide cube
    Wide = readfits(Wname, hdrW, /silent)
