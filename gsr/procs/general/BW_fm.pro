function sixtystr, decimals

;converts decimal hours to AO input format: hhmmsss or ddmmsss
decimals = [decimals]
; deal with < 0
whltz = fltarr(n_elements(decimals))
wh = where(decimals lt 0., ct)
if ct ne 0. then whltz(wh) = 1.
decimals = abs(decimals)
str = strarr(  (size(decimals))[1: (size(decimals))[0] ])

for i = 0, n_elements(decimals)-1 do begin
...
if (n_elements(decimals) eq 1) then str = reform(str)
...
;         13 seconds seems to be the best time to choose.
...
;
;   FITSWHEN - Loops per WAPP fits file. Default is 4. This is mostly irrelevant to 
;           GALSPECT operation, but keeps the WAPPS happy.
;
;   IFLONAME - the name of the iflo.gui file to load. default is 'galfa_iflo.gui'
;   
;   RESTFREQ - The rest frequency to use - if not set, assumed to be 1420.405750
;   
...
for i=0, N-1 do begin
    for j=0, late-1 do begin
        ...
        if keyword_set(fixedaz) then out_exp[i, j] = 'BASKETWEAVE' + ' source=' + name + ' dec=' + sixtystr(lowdec) + ' lst=' + ao_start_ras[i,j]+' loops=' + strcompress(n_reps-j, /remove_all) else out_exp[i, j] = 'BASKETWEAVE' + ' source=' + name + ' ra=' +ao_start_ras[i,j]+ ' dec=' + sixtystr(lowdec) + ' lst=' + ao_start_lsts[i,j]+' loops=' + strcompress(n_reps-j, /remove_all)
...
;while (endloop eq 0)  do begin
    i = days_not_done[0]
                                ; pt1 file
    filenamept1 = sourcename + '_day_' + string(i, format='(I2.2)') + '_pt1'

    openw, 1, filenamept1
    printf, 1, 'RECEIVER alfa'
    printf, 1, 'ALFAANGLE ' + alfarotator 
    printf, 1, 'LOADIF galfa_iflo.gui'
    printf, 1, 'WAPPCONFIG'
    printf, 1, 'CATALOG ' + sourcename + '_sources.list'
    printf, 1, 'SEEK ' + sourcename + '_sfs'
    printf, 1, 'ADJUSTPOWER'
    printf, 1, 'NEWFITSFILE'
    close, 1
    
                                ;pt2 file
    filenamept2 = sourcename + '_day_' + string(i, format='(I2.2)') + '_pt2'

    openw, 1, filenamept2
     printf, 1, 'SMARTFREQ freqs={' + sfsfreqsstring + '} secs=10 loops=2 caltype=hcal'
    if (keyword_set(ifloname)) then  printf, 1, 'LOADIF ' + ifloname else printf, 1, 'LOADIF galfa_iflo.gui'
if keyword_set(fixedaz) then printf, 1, 'SETUP basketweave secs=' + strcompress(sweep_time, /remove_all) + ' declen=' + strcompress(string(ddec,  format='(F6.3)'), /remove_all) + ' turntime=' + strcompress(wait_time, /remove_all) else printf, 1, 'SETUP basketweave rarate=15 secs=' + strcompress(sweep_time, /remove_all) + ' declen=' + strcompress(string(ddec,  format='(F6.3)'), /remove_all) + ' turntime=' + strcompress(wait_time, /remove_all)
     printf, 1, 'SETUP basketweave ncals=1 calwhen="top and bottom of each" calsecs=3'
if keyword_set(fixedaz) then printf, 1, 'SETUP basketweave mode = azza caltype=hcal dop=each dopl=never fitswhen=' + lpf else printf, 1, 'SETUP basketweave caltype=hcal dop=each dopl=never fitswhen=' + lpf
