...
pro stray_rad, scb, todfile, mhc, a, d, angle, spbox, etaset=etaset, outfile=outfile
...
; main beam effciency:
if keyword_set(etaset) then eta =  [0.875729, 0.764944,0.743301,0.742042,0.764944,0.769971,0.770685] else eta = fltarr(7)+1.
...
for i=0, n_sec-1 do begin
    ...
    for j=0, 6 do begin
       ...
       ; find a region of the sky around the point
        a0 = interpol(findgen(512), a[*, 0], mhc[i+q].ra_halfsec[j]*15.+0.15) > 0.
        a1 = interpol(findgen(512), a[*, 0], mhc[i+q].ra_halfsec[j]*15.-0.15) < 511.
        d0 = interpol(findgen(512), d[0, *], mhc[i+q].dec_halfsec[j]-0.15) > 0.
        d1 = interpol(findgen(512), d[0, *], mhc[i+q].dec_halfsec[j]+0.15) < 511.
   
        if ((a0 gt 510) or (a1 lt 1) or (d0 gt 510) or (d1 lt 1)) then continue
        ...
        galfabeam_reconstruct, j, bp, (a_prox-mhc[i+q].ra_halfsec[j]*15.)*cos(meandec*!pi/180.)*60., (d_prox-mhc[i+q].dec_halfsec[j])*60., tb, mb, sl, /noreform, rotang=angle[i+q]+180        
   
        data_slice = data[a0:a1,d0:d1, 1024]
...
; not really be the whole beam) but not much will  anyway, so to hell with it.
        ...
        sp = total(total(rebin(reform(sl, sz[1], sz[2], 1), sz[1], sz[2], 2048)*data[a0:a1,d0:d1, *], 1), 1)/scale_factor/eta[j]
        spbox[*, i, j] = sp
