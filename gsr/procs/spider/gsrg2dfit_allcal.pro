pro gsrg2dfit_allcal, sigmalimit, $
...
;	for alfa: 24nov2004. changed convergence criterion. can't fig
;why the old one worked at all!
;-

sigmaprevious= 1.0e12
...
delt = a[ 9]
adelt = abs(delt)
adelt = 1.0 < adelt  ;original for interim correlator and lbw feed
adelt = 1.0*dtsys_dza < adelt  ;for alfa
...
;NEW CONVERGENCE CRITERION OF 24NOV2004: STOP IF SIGMA CHANGES BY LT .1%
g2dcurv_allcal, delt_zaencoder, az_scan, za_scan, totoffset, $
        hgt_lobe, cen_lobe, wid_lobe, tsys, dtsys_dza, $
        tsrc, az_bmcntr, za_bmcntr, bmwid_0, bmwid_1, phi_bm, $
...
sigma = sqrt( sigsq)
delsig_frac= (sigmaprevious-sigma)/sigma

;print, nloop, a, sigma, delsig_frac, format='(i3,11f10.3, f10.5)'
;STOP

if delsig_frac lt (ax1/10.) then goto, finished
sigmaprevious= sigma
...
;	stop, 'STOP IN G2DFIT_ALLCAL WITH NLOOP GE 100'
		problem = -2
		goto, PROBLEM
endif
