...
pro gsrgfit_allcal, sigmalimit, xdata, tdata, $
...
;
;HISTORY:
;26nov04: changed halfassed to 0.25; changed convergence criterion to looking
;at fractional change in siama.
;-
...
if delsig_frac lt (ax1/10.) then redoit= 0
sigmaprevious= sigma
...
;print, nloop, transpose(a)
;print, halfassed
;print, nloop, hgt1, cen1, wid1, sigma, delsig_frac, format='(i3, 10f10.4, f10.7)'
...
;	if (halfassed lt .005) then begin
		problem = -2
	print, 'AT PROBLEM -2 CHECK IN GFIT_ALLCAL'
;		GOTO, PROBLEM
		GOTO, FINISHED
;	endif
...
;IN THE PAST, WE SET DERIVED VALS EQUAL TO INPUT ONES. TRY NOT DOING THIS...

IF (PROBLEM NE -2) THEN BEGIN
tsys1 = tsys0
slope1 = slope0
alpha1 = alpha0
hgt1 = hgt0
