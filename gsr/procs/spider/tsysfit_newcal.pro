...
pro tsysfit_newcal, nrst, offset, pwr, $              
        tfit_final, sigma, stripfit, sigstripfit, problem, cov
...
;
;INPUTS:
;	nrst, the sequential number of the strip (0 to NRSTRIPS-1).
;
...
;
;OUTPUTS:
;	TFIT_final, the fitted line to the datapoints.
;
;	SIGMA, the rms of the residuals (PWR- TFIT_final)
;
...
;If problem eq 0, then it found an acceptable solution with either zero,
;one, or two sidelobes. if problem ne 0, the nonlinear fit couldn't even 
;converge for a single gaussian.
;
...
gsrgfit_allcal, sigmalimit, offset, pwr, $
...
;stop, 'IN 3 GAUSSIAN RESULT IN TSYSFIT_NEWCAL'

;PRINT, 'PASSED THE 3-GAUSSIAN FIT SUCCESSFULLY'
...
;print, 'problemdx0, problemdx1 = ', problemdx0, problemdx1
...
;print, 'problem in single gauss fit = ', problem
...
stripfit[ 0, nrst] = alpha_final
stripfit[ 1:3, nrst] = hgt_final[ indxorder]
stripfit[ 4:6, nrst] = cen_final[ indxorder]
stripfit[ 7:9, nrst] = wid_final[ indxorder]
stripfit[ 10, nrst] = tsys_final  
stripfit[ 11, nrst] = slope_final

sigstripfit[ 0, nrst] = sigalpha_final
sigstripfit[ 1:3, nrst] = sighgt_final[ indxorder]
sigstripfit[ 4:6, nrst] = sigcen_final[ indxorder]
sigstripfit[ 7:9, nrst] = sigwid_final[ indxorder]
sigstripfit[ 10, nrst] = sigtsys_final  
sigstripfit[ 11, nrst] = sigslope_final
